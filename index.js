...
module.exports = function (params) {
  ...
  // const prepend = "(function(){\n'use strict';\nvar module = {};\n  var exports = {};\n  ";
  const PREPEND_LINES = 3;
  const APPEND = "\n  return module.exports || exports;\n})() ";
  const APPEND_LINES = 2;
  ...
  function processInclude(content, filePath, sourceMap) {
    // var matches = content.match(/require\(['"](.+$)['"]\)/mg);
    var matches = content.match(/require\((.+?)\)/mg);
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      var includePath = split[1];
      if (includePath.indexOf('.js', includePath.length - '.js'.length) == -1) {
        includePath += '.js';
      }
      ...
      if (includePaths != false) {
        ...
        for (var y = 0; y < includePaths.length; y++) {
          var newPath = includePaths[y] + "/" + includePath;
          var globResults = glob.sync(newPath, {mark: true});
          fileMatches = fileMatches.concat(globResults);
        }
      }else{
        // Otherwise search relatively
        includePath = relativeBasePath + "/" + includePath;
      ...
      for (var y = 0; y < fileMatches.length; y++) {
        ...
        var resultContent = PREPEND + result.content + APPEND;
          ...
          if (result.map) { // result had a map, merge mappings
            ...
            if (result.map.mappings && result.map.mappings.length > 0) {
              ...
              resultMap.eachMapping(function (mapping) {
                ...
                map.addMapping({
                  generated: {
                    line: mapping.generatedLine + currentLine - 1 + PREPEND_LINES,
